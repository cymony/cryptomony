...
func TestRistrettoSmallMultiplesTestVectors(t *testing.T) {
	...
	basepointMultiple := newElement().Identity()
	ristrettoBasepoint := newElement().Base()

	for i := range testVectors {
		// Grab the bytes of the encoding
		encoding, err := hex.DecodeString(testVectors[i])
		test.CheckNoErr(t, err, fmt.Sprintf("#%d: bad hex encoding in test vector", i))

		// Decode the test vector to a ristretto255 element
		decodedPoint := newElement()

		err = decodedPoint.Decode(encoding)
		test.CheckNoErr(t, err, fmt.Sprintf("#%d: could not decode test vector", i))

		// Re-encode and check round trips
		roundtripEncoding := decodedPoint.Encode()
		...
		computedEncoding := basepointMultiple.Encode()
		...
		basepointMultiple.Add(ristrettoBasepoint)
	}
}
...
func TestRistrettoBadEncodingsTestVectors(t *testing.T) {
	...
	basepointMultiple := newElement()
	basepointMultiple.Identity()
...
func TestRistrettoFromUniformBytesTestVectors(t *testing.T) {
	...
	element := cvtEl(newElement())

	for i, input := range inputs {
		hash := sha512.Sum512([]byte(input))
		_, err := element.SetUniformBytes(hash[:])
		test.CheckNoErr(t, err, "could not set uniform bytes")

		if encoding := hex.EncodeToString(element.Encode()); encoding != elements[i] {
...
func TestEquivalentFromUniformBytes(t *testing.T) {
	...
	for i, input := range inputs {
		...
		_, err = element.SetUniformBytes(h)
		test.CheckNoErr(t, err, "could not set uniform bytes")

		if encoding := hex.EncodeToString(element.Encode()); encoding != expected {
...
func TestMarshalElement(t *testing.T) {
	...
	xbytes := sha512.Sum512([]byte("Hello World"))
	_, err := x.SetUniformBytes(xbytes[:])
	test.CheckNoErr(t, err, "could not set unifrom bytes")
